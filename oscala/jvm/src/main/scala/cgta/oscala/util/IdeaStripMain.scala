...
// Created by bjackman @ 10/7/15 3:24 PM
//////////////////////////////////////////////////////////////

object IdeaStripMain {
  ...
  object PPRINT {def |(s: Any) = Console.println(s)}
  object INFO {
    def |(s: Any) = {
      Console.print("<INF> ")
      Console.println(s)
  ...
  class IdeaStrip(targetDirs: List[File]) {

    val projects = targetDirs.map(d => Project(d))
    ...
    def start() {
      ...
      projects.foreach(_.start(MaxLen
      ))
      while (true) {
        Thread.sleep(60000);
      }
  ...
  object LineParsers {
    def stackElementify(packPath: String, clsName: String, lnum: String, msg: String): String = {
      val pkg = packPath.replaceAll("/", ".")
      "<<< at " + pkg + "." + clsName + ".some_method(" + clsName + ".scala:" + lnum + ") >>> "
    }
  ...
  case class IdeaParse(p: Path, maxProjectNameLen: Int) {
    var lastWriteSecs = System.nanoTime() / 1e9.toLong
    var justPrintedInitialChanges = false
    val fallAsleepSecs = 10L
    var needsSleepHeader = false
    def log(s: Line) = {
      val tstamp = (new SimpleDateFormat("HH:mm:ss")).format(new Date())
      val res = new StringBuilder
      res.append(tstamp).append(" ")
      res.append("(").append(("%-" + maxProjectNameLen + "s").format(p.name)).append(") ")
      res.append(s.lvl).append(" ")
      //    res.append("[").append(s.msg).append("]")
      res.append(s.msg)
      if (needsSleepHeader) {
        needsSleepHeader = false
        PPRINT | " "
        PPRINT | ("#" * 80)
        PPRINT | " "
      }
    ...
    def apply(s: String) {
      val curSecs = System.nanoTime() / 1e9.toLong
      val quietSecs = curSecs - lastWriteSecs
      lastWriteSecs = curSecs
      if (quietSecs > fallAsleepSecs) {
        needsSleepHeader = true
      }

      Line.parse(stripANSI(s)) match {
        case Some(line) =>
          val res = LineParsers.filepath(line) match {
            case Nil =>
              if (LineParsers.initialChanges(line) && !justPrintedInitialChanges) {
                ...
                LineParsers.fallback(line).foreach(log _)
              }
  ...
  case class Project(targetDir: File) {
    val name = {
      targetDir.getParentFile.getName match {
        case "." => "root"
        case "jvm" | "js" => targetDir.getParentFile.getParentFile.getName + "-" + targetDir.getParentFile.getName
        case s => s
      }
    }

    def streamsPath = Path(name, s"$targetDir/streams/")
    def globalPath = Path("global", "$global/streams/$global/out")
    def globalPath2 = Path("global", "$global/$global/$global/streams/out")
    def testPath = Path("test", "test/compile/$global/out")
    def testPath2 = Path("test", "test/compile/$global/streams/out")
    def compilePath = Path("compile", "compile/compile/$global/out")
    def compilePath2 = Path("compile", "compile/compile/$global/streams/out")

    def subpaths = List(globalPath, testPath, compilePath)
    def subpaths2 = List(globalPath2, testPath2, compilePath2)
    def allPaths = (subpaths ++ subpaths2).map(streamsPath / _)

    def start(maxProjectNameLen: Int) = allPaths.map { p =>
      val parser = IdeaParse(p, maxProjectNameLen)
      TailThread(p)(parser.apply _).start
    }
      ...
      try {
        ...
        def loop {
          ...
          if (c == '\n'.toInt) {
            ...
            //-1 is DEAD LAND
            PPRINT | p + " REACHED EOF ??? " + buf.toString
          }
        }
        loop
      } finally {
        PPRINT | "EXITING Watcher of " + p
      }
    }
  ...
  //********************************************************
  //********************************************************
  //NOTE THAT TO GET THIS TO WORK YOU HAVE TO ADD
  // Main-Class: cgta.oscala.util.IdeaStripMain
  // into the jar made with the assembly task.
  //********************************************************
  //********************************************************
  //********************************************************
  //********************************************************
  ...
  def main(args: Array[String]) {
    val root = args.getOpt(0).getOrElse(".").toFile

    val maxDepth = 3
    def getTargetsLoop(f: File, depth: Int): List[File] = {
      if (depth <= maxDepth) {
        if (f.isDirectory) {
          if (f.getName == "target") {
            List(f)
            ...
            Nil
          } ::: f.listFiles().flatMap(getTargetsLoop(_, depth + 1)).toList
        } else {
          Nil
        }
      } else {
    ...
    val targets = getTargetsLoop(root, 0)

    PRINT | s"Watching under ${root.getAbsolutePath}"
    PRINT | s"TARGETS $targets"

    //Find all the output directories under the current directory
    new IdeaStrip(targets).start()
  }
}