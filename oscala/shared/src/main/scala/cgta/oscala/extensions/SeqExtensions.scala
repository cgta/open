...
class SeqExtensions[A](val xs: Seq[A]) extends AnyVal {
  ...
  def weightedAvgOpt[B: Numeric, C: Numeric](toValue: A => B, toWeight: A => C): Option[Double] = {
    val numV = implicitly[Numeric[B]]
    val numW = implicitly[Numeric[C]]
    var totWV = 0.0
    var totW = 0.0
    xs.foreach { x =>
      val v = numV.toDouble(toValue(x))
      val w = numW.toDouble(toWeight(x))
      totWV += v * w
      totW += w
    }
