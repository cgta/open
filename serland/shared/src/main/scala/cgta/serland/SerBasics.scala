...
object SerBasics {
  ...
  trait LowSerClasses {
    class ArraySerClass[A: SerClass: ClassTag] extends SerClass[Array[A]] {
      implicit val sca = implicitly[SerClass[A]]
      override def write(a: Array[A], out: SerOutput): Unit = out.writeIterable(a, sca)
      override def read(in: SerInput): Array[A] = in.readIterable(sca).toArray
      override def schema: SerSchema = SerSchemas.XSeq(SerSchemas.XList, sca.schema.schemaRef)
      override def gen: Gen[Array[A]] = {
        implicit val a = Arbitrary(sca.gen)
        Arbitrary.arbitrary[Array[A]]
      }
    }
    implicit def arraySerClass[A: SerClass: ClassTag] = new ArraySerClass[A]
  }

  trait SerClasses extends LowSerClasses {
    ...
    implicit object UnitSerClass extends SerClass[Unit] {
      override def write(a: Unit, out: SerOutput) = {}
      override def read(in: SerInput): Unit = {}
      override def schema: SerSchema = SerSchemas.XUnit()
      override def gen: Gen[Unit] = Arbitrary.arbitrary[Unit]
    }
