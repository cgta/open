 ...
 *
 */
class SerPennyIn(ins: InputStream, override val isHumanReadable: Boolean = false) extends SerInput {
  val IS = BinaryHelp.FromInputStream
...
//    doInitialCheck()
  ...
  override def readSerInput(): () => SerInput = {
    ...
    () => new SerPennyIn(new ByteArrayInputStream(bs))
  }
