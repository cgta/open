 ...
 */
class SerPennyUnparsed(val in: InputStream, val initWt: Int) {
  val O = BinaryHelp.ToOutputStream
  val I = BinaryHelp.FromInputStream
  implicit val ins = in
  implicit val out = new ByteArrayOutputStream()
  def slurp() {
    O.writeUVar(initWt)
    dispatchOnWt(initWt)
  }
