...
sealed trait Gen[+T] {
  ...
  private[gen] def doApply(p: P): R[T]
...
object Gen {
  ...
  private[gen] def r[T](r: Option[T]): R[T] = new R[T] {
    val result = r
  }

  /** Generator factory method */
  private[gen] def gen[T](f: P => R[T]): Gen[T] = new Gen[T] {
    def doApply(p: P) = f(p)
  }
  ...
  trait Parameters {
     ...
     *  value of this parameter is controlled by Test.parameters.minSize. */
    val size: Int
   ...
   *  [[cgta.serland.gen.util.Buildable]] instance. The elements in the container will
   ...
   *  implicit [[cgta.serland.gen.util.Buildable]] instance. The elements in the
   ...
   *  exists an implicit [[cgta.serland.gen.util.Buildable]] instance. The
  ...
  /** Generates a container of any Traversable type for which there exists an
   *  implicit [[cgta.serland.gen.util.Buildable2]] instance. The elements in
