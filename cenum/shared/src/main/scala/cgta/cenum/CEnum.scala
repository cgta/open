...
trait CEnum {self =>
  ...
  trait EnumElement extends Ordered[EnumElement] {
    ...
    lazy val elementName : String = toString
    ...
    def is(that : EET) = this.ordinal == that.ordinal
    def isnt(that : EET) = this.ordinal != that.ordinal
  ...
  final lazy val toIMap: IMap[String, EET] = IMap(toIVec.map(el => el.elementName -> el): _*)
  ...
  implicit lazy val ser: SerClass[EET] = new SerClass[EET]() {
    ...
    override lazy val schema = {
      import SerSchemas._
      XEnum(XCEnum, toIVec.map(ee => XEnumElement(ee.elementName, ee.ordinal)))
    }
